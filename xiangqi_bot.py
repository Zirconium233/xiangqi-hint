...
PIKAFISH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "pikafish" + (".exe" if os.name == "nt" else ""))
...
class Bot:
    ...
    def load_cnn(self):
        ...
        try:
            ...
            if os.path.exists(MODEL_PATH):
                ...
                print(f"  CNN model loaded! (PyTorch/{self.cnn.device})")
                return True
        except Exception as e:
    ...
    def parse_board_cnn(self, img):
        """Parse one trustworthy board frame.

        A low-confidence or FEN-repaired frame is never allowed to become a
        game state. It is independently captured a second time and must agree
        cell-for-cell with the first frame. This prevents an animation or a
        window drag from being turned into a plausible but false position.
        """
        if not self.cnn:
            return None
        ...
        def read_once(frame, prefix):
            board = self.cnn.parse_board(
                frame, self.cols_logical, self.rows_logical,
                self.retina_scale, self.win_x, self.win_y,
                self.cell_w, self.cell_h, debug_dir=None,
                debug_prefix=prefix)
            probs = getattr(self.cnn, '_cell_probs', None)
            min_any = 1.0
            min_piece = 1.0
            complete = probs is not None
                ...
                for r in range(10):
                    for c in range(9):
                        ...
                        if p is None:
                            ...
                            continue
                        conf = float(p.max())
                        min_any = min(min_any, conf)
                        if board[r][c] is not None:
                            min_piece = min(min_piece, conf)
            kings = sum(p == 'K' for row in board for p in row)
            kings += sum(p == 'k' for row in board for p in row)
            sane = (complete and kings == 2 and
                    sum(p == 'K' for row in board for p in row) == 1 and
                    sum(p == 'k' for row in board for p in row) == 1)
            corrections = int(getattr(self.cnn, '_last_validation_changes', 0))
            quality = {
                'sane': sane,
                'corrections': corrections,
                'repairs': tuple(getattr(self.cnn, '_last_validation_cells', ())),
                'occupancy_overrides': tuple(
                    getattr(self.cnn, '_last_occupancy_overrides', ())),
                'min_any': min_any,
                'min_piece': min_piece,
            }
            return board, quality

        board1, q1 = read_once(img, debug_prefix)
        # An uncorrected, reasonably confident frame can enter the candidate
        # stabilizer immediately. Anything weaker must agree with a fresh shot.
        needs_retry = (not q1['sane'] or q1['corrections'] > 0 or
                       q1['min_any'] < 0.90 or q1['min_piece'] < 0.90)
        if not needs_retry:
            self._last_parse_quality = q1
            return board1

        generation = getattr(self, '_geometry_generation', None)
        time.sleep(0.12)
        try:
            ...
            return None
        if generation is not None and generation != getattr(self, '_geometry_generation', None):
            self._last_parse_quality = None
            return None
        board2, q2 = read_once(img2, f"s{step:03d}b_")
        same = all(board1[r][c] == board2[r][c]
                   for r in range(10) for c in range(9))
        repairs = tuple(set(q1['repairs']) | set(q2['repairs']))
        repair_stable = (tuple(sorted(q1['repairs'])) ==
                         tuple(sorted(q2['repairs'])))
        # Windows tracking may inspect empty-only repairs as hypotheses.
        # A small, repeated non-king relabel is also usable for bootstrap:
        # the board validator already enforces piece-count limits, and the
        # caller still validates the resulting complete position with the
        # engine. Large or king-related repairs remain rejected.
        repair_candidate = (
            getattr(self, '_allow_empty_repairs', False) and
            0 < len(repairs) <= 2 and repair_stable and
            all(old not in ('K', 'k') and new not in ('K', 'k')
                for _r, _c, old, new in repairs) and
            len(repairs) >= max(q1['corrections'], q2['corrections']))
        if (not same or not q2['sane'] or
                ((q1['corrections'] > 0 or q2['corrections'] > 0) and not repair_candidate) or
                q2['min_any'] < 0.55 or q2['min_piece'] < 0.65):
            self._last_parse_quality = None
            return None
        q2['repairs'] = repairs
        q2['corrections'] = len(repairs)
        q2['repair_stable'] = repair_stable
        self._last_parse_quality = q2
        return board2
...
if __name__ == '__main__':
    raise SystemExit('此文件仅作为识别基础库使用。请运行 python -X utf8 win_hint_gui.py；本项目不提供自动下棋入口。')
