...
class PieceClassifierCNN:
    ...
    def __init__(self, model_path=MODEL_PATH):
        if torch.cuda.is_available():
            ...
            # All inference tensors have the same 48x48 shape. cuDNN can
            # select and reuse the fastest fixed convolution kernels.
    ...
    def _classify_cell_full(self, patch):
        ...
        return self._classify_patches_full([patch])[0]

    def _classify_patches_full(self, patches, batch_size=128):
        """Classify many cell crops in batches on the selected accelerator."""
        if not patches:
            return []
        tensors = []
        for patch in patches:
            resized = cv2.resize(patch, (CELL_SIZE, CELL_SIZE))
            rgb = cv2.cvtColor(resized, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
            tensors.append(rgb.transpose(2, 0, 1))
        data = torch.from_numpy(np.ascontiguousarray(np.stack(tensors)))
        results = []
        with torch.inference_mode():
            for start in range(0, len(tensors), batch_size):
                batch = data[start:start + batch_size].to(self.device, non_blocking=True)
                probs_batch = torch.softmax(self.model(batch), dim=1).cpu().numpy()
                for probs in probs_batch:
                    pred = int(probs.argmax())
                    cls = CLASSES[pred]
                    results.append((None if cls == '_' else cls,
                                    float(probs[pred]), probs))
        return results
    ...
    def parse_board(self, img, cols_logical, rows_logical, retina_scale, win_x, win_y,
        ...
        for r in range(10):
            for c in range(9):
                ...
                cells.append((r, c, px, py, patch))

        # One accelerator call for the whole board instead of one Python /
        # CUDA launch per square.
        large_results = self._classify_patches_full([cell[4] for cell in cells])
        compact_jobs = []
        for cell, result in zip(cells, large_results):
            r, c, px, py, patch = cell
            piece, conf, probs = result
            board[r][c] = piece
            self._cell_probs[r][c] = probs
            self._cell_patches[r][c] = patch
            if (piece is not None and compact_radius >= 12 and
                    conf < 0.78 and
                    float(probs[CLASS_TO_IDX['_']]) >= 0.25):
                cx1, cy1 = max(0, px - compact_radius), max(0, py - compact_radius)
                cx2, cy2 = min(w, px + compact_radius), min(h, py + compact_radius)
                compact = img[cy1:cy2, cx1:cx2]
                if compact.shape[0] >= 20 and compact.shape[1] >= 20:
                    compact_jobs.append((r, c, piece, conf, compact))

        # Usually empty. Only ambiguous candidates pay for the second scale.
        compact_results = self._classify_patches_full(
            [job[4] for job in compact_jobs])
        for job, result in zip(compact_jobs, compact_results):
            r, c, piece, conf, _compact = job
            compact_piece, compact_conf, _compact_probs = result
            if compact_piece is None and compact_conf >= 0.55:
                self._last_occupancy_overrides.append(
                    (r, c, piece, float(conf), float(compact_conf)))
                board[r][c] = None

        if debug_dir:
            for (r, c, _px, _py, patch), result in zip(cells, large_results):
                piece, conf, _probs = result
                label = board[r][c] if board[r][c] else '_'
                subdir = PIECE_TO_DIR[label]
                fname = f"{prefix}r{r}c{c}_{conf*100:.0f}.png"
                cv2.imwrite(os.path.join(debug_dir, subdir, fname), patch)

        self._last_validation_changes = 0
        ...
        for piece, limit in PIECE_LIMITS.items():
            ...
            for r, c, conf in excess:
                ...
                for idx in np.argsort(probs)[::-1]:
                    ...
                    old_label = board[r][c]
                    board[r][c] = alt_piece
                    piece_cells[piece] = [cell for cell in piece_cells[piece]
                                          if cell[:2] != (r, c)]
                    alt_conf = float(probs[idx])
                    repairs.append((r, c, old_label, alt_piece))
                    if alt_piece:
                        piece_cells.setdefault(alt_piece, []).append((r, c, alt_conf))
                    changes += 1
                    break

        signature = tuple(repairs)
        if changes and signature != getattr(self, '_validation_signature', None):
            print(f"    FEN validation: proposed {changes} repair(s): {repairs}")
        self._validation_signature = signature
        self._last_validation_cells = repairs
        self._last_validation_changes = changes
        return board
